<template>
    ...
    <Sidebar :links="links">
        <div v-if="$page.props.auth.user.permissions.includes('staff.create')" style="margin-bottom: 20px">
            <button v-if="$page.props.auth.user.permissions.includes('staff.create')" class="primary">Добавить сотрудника</button>
        </div>
        <table v-if="users.data.length !== 0" @click="actionsOpened = {}; isSettingOpened = false">
            <thead>
                <tr>
                    <th @click.stop style="display: flex; align-items: center; justify-content: center">
                        <svg @click="isSettingOpened = !isSettingOpened" class="settings" width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <transition name="fade">
                            <div v-if="isSettingOpened" class="settings-buttons">
